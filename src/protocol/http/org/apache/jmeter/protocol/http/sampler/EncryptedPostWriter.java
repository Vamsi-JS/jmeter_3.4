 ...
 * body of the POST.
 */
public class EncryptedPostWriter extends PostWriter {
	...
	/** The Encrypt form data that is going to be sent in post body */
	protected byte[] encryptformDataPostBody;

	/** The boundary string for multipart */
	private final String boundary;

	private static final Logger log = LoggerFactory.getLogger(EncryptedPostWriter.class);

	/**
	 * Constructor for PostWriter. Uses the PostWriter.BOUNDARY as the boundary
	 * string
	 *
	 */
	 ...
	 */
	public EncryptedPostWriter(String boundary) {
	 ...
	 * @return the post body sent. Actual file content is not returned, it is just
	 *         shown as a placeholder text "actual file content"
	 * @throws IOException
	...
	public String sendPostData(URLConnection connection, HTTPSamplerBase sampler) throws IOException {
		...
		if (sampler.getUseMultipartForPost()) {
			...
			if (sampler.getArguments() != null && !sampler.hasArguments() && sampler.getSendFileAsPostBody()) {
				...
				OutputStream out = connection.getOutputStream();
				out.write(encryptformDataPostBody);
				log.info("formDataUrlEncoded ==========================> {} ",new String(formDataUrlEncoded));
				out.flush();
				out.close();
	...
	private void encryption() {
		encryptformDataPostBody = "This is temp string".getBytes();
		...
		if (sampler.getUseMultipartForPost()) {
			...
			for (JMeterProperty jMeterProperty : sampler.getArguments()) {
				...
				writeFormMultipart(bos, parameterName, arg.getValue(), contentEncoding,
						sampler.getDoBrowserCompatibleMultipart());
			}
			...
			// If there are no arguments, we can send a file as the body of the request
			if (sampler.getArguments() != null && sampler.getArguments().getArgumentCount() == 0
	...
	/**
	 * Write the start of a file multipart, up to the point where the actual file
	 * content should be written
	 */
	private void writeStartFileMultipart(OutputStream out, String filename, String nameField, String mimetype)
			throws IOException {
	 ...
	 *
	 * @param filename
	 *            the filename of the file to write to the stream
	 * @param out
	...
	private void writeFormMultipart(OutputStream out, String name, String value, String charSet,
			boolean browserCompatibleMultipart) throws IOException {
