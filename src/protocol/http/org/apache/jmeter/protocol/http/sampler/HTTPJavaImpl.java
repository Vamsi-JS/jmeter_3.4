...
public class HTTPJavaImpl extends HTTPAbstractImpl {
    ...
    private static final boolean ENABLE_HTTP_ENCRYPTION =
            JMeterUtils.getPropDefault("httpsampler.enable_http_encryption", false); // $NON-NLS-1$
    
    
    static {
        log.info("Maximum connection retries = {}", MAX_CONN_RETRIES); // $NON-NLS-1$
        log.info("Enable HTTP Encryption = {}", ENABLE_HTTP_ENCRYPTION); // $NON-NLS-1$
    }
    ...
    protected void setPostHeaders(URLConnection conn) throws IOException {
        if(ENABLE_HTTP_ENCRYPTION)
        {
        	postOrPutWriter = new EncryptedPostWriter();	
            log.info("Created EncryptedPostWriter : ");
        }
        else
        {
    ...
    protected HTTPSampleResult sample(URL url, String method, boolean areFollowingRedirect, int frameDepth) {
        ...
        try {
            ...
            if(postOrPutWriter instanceof EncryptedPostWriter)
            {
            String randomKey =	((EncryptedPostWriter) postOrPutWriter).getRandomkey();
